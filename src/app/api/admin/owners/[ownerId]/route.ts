...
export async function PUT(
  ...
  try {
    ...
    if (
      ...
      (username && username !== existingOwner.username)
    ) {
...
export async function DELETE(
  ...
  try {
    ...
    // Check if owner exists and get foodcourts via permissions instead
    const owner = await db.user.findUnique({
      where: { id },
      include: {
        permissions: {
    ...
    // Check if owner has any foodcourts assigned as owner
    const ownedFoodcourts = await db.foodcourt.findMany({
      where: { ownerId: id },
    });

    if (ownedFoodcourts.length > 0) {
