// ~/src/app/api/admin/permissions/history/route.ts
...
// Handler untuk mendapatkan history perubahan permission
export async function GET(req: NextRequest) {
  try {
    ...
    // Filter untuk permission (join dengan OwnerPermission)
    if (ownerId || foodcourtId) {
      filter.permission = {};

      if (ownerId) {
        filter.permission.ownerId = ownerId;
      }
    ...
    return NextResponse.json(
      { error: "Failed to get permission history" },
      { status: 500 }
    );
  }
}